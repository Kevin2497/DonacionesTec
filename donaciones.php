...
// Función para obtener resultados por tipo de donación
function obtenerDonacionesPorTipo($conexion, $tipo) {
    $stmt = $conexion->prepare("SELECT u.idUsuario, u.nombre, d.idDonacion, d.tipoDonacion, d.estado 
        FROM usuarios u 
        JOIN donaciones d ON u.idUsuario = d.idUsuario 
        WHERE d.tipoDonacion = ? AND u.rol = 'usuario'");
...
$monetarias = obtenerDonacionesPorTipo($conexion, "monetaria");
$especie = obtenerDonacionesPorTipo($conexion, "especie");
$propuestas = obtenerDonacionesPorTipo($conexion, "propuesta");
?>
        ...
        <?php if ($monetarias->num_rows > 0): ?>
            <div class="tabla-scroll">
                <table class="tabla1">
                    ...
                    <tbody>
                        <?php while ($fila = $monetarias->fetch_assoc()): ?>
                        <tr>
        ...
        <?php if ($especie->num_rows > 0): ?>
            <div class="tabla-scroll">
                <table class="tabla2">
                    ...
                    <tbody>
                        <?php while ($fila = $especie->fetch_assoc()): ?>
                        <tr>
                            <td><?= htmlspecialchars($fila['nombre']) ?></td>
                            <td><?= htmlspecialchars($fila['tipoDonacion']) ?></td>
